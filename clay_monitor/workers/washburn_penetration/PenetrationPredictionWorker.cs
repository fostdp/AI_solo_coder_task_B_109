...
namespace WashburnPenetration;

public class PenetrationPredictionWorker : BackgroundService, IPenetrationPredictionService
{
    ...
    public PenetrationPredictionWorker(IMessageBus bus, IOptions<PenetrationPredictionOptions> options)
    {
        _bus = bus;
    ...
    {
        var results = new PenetrationResult[inputs.Length];
        await Parallel.ForEachAsync(inputs, ct, async (input, token) =>
        {
            var index = Array.IndexOf(inputs, input);
            results[index] = await PredictAsync(input, token);
        });
        return results;
    }

    public async Task<PenetrationResult[]> ParallelPredictBatchAsync(PenetrationInput[] inputs, CancellationToken ct = default)
    {
        var results = new PenetrationResult[inputs.Length];
        ...
        await Parallel.ForEachAsync(inputs, parallelOptions, async (input, token) =>
        {
            var index = Array.IndexOf(inputs, input);
            results[index] = await PredictAsync(input, token);
        });
