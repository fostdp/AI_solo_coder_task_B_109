...
namespace ClayMonitor.VirtualReinforcement;

public record VirtualReinforcementInput
{
...
public record VirtualReinforcementResult
{
    public int SculptureId { get; init; }
    public string MaterialName { get; init; } = string.Empty;
...
public interface IVirtualReinforcementService
{
    Task<VirtualReinforcementResult> SimulateAsync(VirtualReinforcementInput input, CancellationToken ct = default);
    Task<VirtualReinforcementResult[]> CompareMaterialsAsync(
        int sculptureId,
        string[] materialNames,
        double porosity,
        CancellationToken ct = default);
    Voxel3D[] Generate3DVoxels(VirtualReinforcementInput input, double[] depthProfile, double[] glossProfile);
    double[] CalculateGlossProfile(double[] depthProfile, double originalGloss, double materialGloss);
}

public class VirtualReinforcementService : BackgroundService, IVirtualReinforcementService
{
    ...
    public VirtualReinforcementService(
        IMessageBus bus,
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        await foreach (var penetrationResult in _bus.SubscribeAsync<PenetrationPredictionCompleted>(stoppingToken))
        {
            try
            {
                ...
                await _bus.PublishAsync(new VirtualReinforcementApplied
                {
    ...
    public async Task<VirtualReinforcementResult> SimulateAsync(VirtualReinforcementInput input, CancellationToken ct = default)
    {
        if (!MaterialVisuals.TryGetValue(input.MaterialName, out var visuals) ||
            !PenetrationPredictionService.TryGetMaterialProperties(input.MaterialName, out var matProps))
        {
            return await Task.FromResult(new VirtualReinforcementResult
            {
                SculptureId = input.SculptureId,
                MaterialName = input.MaterialName,
                EnhancementSuggestions = new[] { "未知材料，无法进行虚拟加固模拟" },
    ...
    public async Task<VirtualReinforcementResult[]> CompareMaterialsAsync(
        int sculptureId,
        string[] materialNames,
        double porosity,
        CancellationToken ct = default)
    {
        var results = new List<VirtualReinforcementResult>();
    ...
    public Voxel3D[] Generate3DVoxels(VirtualReinforcementInput input, double[] depthProfile, double[] glossProfile)
    {
    ...
    private string GenerateVisualizationConfig(VirtualReinforcementInput input, MaterialVisualProperties visuals, PenetrationResult penResult)
    {
        var config = new
        {
            mode = input.ViewMode,
    ...
    private string[] GenerateEnhancementSuggestions(double avgPen, double glossChange,
        double reinforcedPct, MaterialVisualProperties visuals, VirtualReinforcementInput input)
    {
